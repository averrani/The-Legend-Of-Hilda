...
int WindowTXT::getTile(int x, int y) {
    return win[x + dimx*y];
}
...
void termInit()      // configure la saisie : ne pas afficher les caracteres tapes
{
    ...
    tcsetattr(STDIN_FILENO, 0, &ttystate);
...
WindowTXT::WindowTXT (int dx, int dy) {
    dimx = dx;
    dimy = dy;
...
char WindowTXT::getCh() { // lire un caractere si une touche a ete pressee
    char touche=0;
#ifdef _WIN32
    if (kbhit())
    {
